...
/*
 * thread - 새롭게 생성된 쓰레드 안에서 클라이언트와 통신 수행한다.
 */
void *thread(void *vargp)
{
...
void doit(int connfd)
{
    ...
    /* 프록시 서버가 엔드 서버로 보낼 정보 파싱 */
    parse_uri(uri, hostname, path, &port);

    /*build the http header which will send to the end server*/
    /* 프록시 서버가 엔드 서버로 보낼 요청 헤더 만든다 endserver_http_header가 위에서 파싱한정보로 채워진다 */
    ...
    /* 엔드 서버에 HTTP 요청 헤더 보낸다 */
    Rio_readinitb(&server_rio, end_serverfd);
    /*write the http header to endserver*/
    /* http 헤더 엔드서버에 쓴다 fd 참조*/
...
void build_http_header(char *http_header, char *hostname, char *path, int port, rio_t *client_rio)
{
    ...
    /* 클라이언트 요청 헤더들에서 Host header와 나머지 헤더들 구분해서 넣어준다 */
